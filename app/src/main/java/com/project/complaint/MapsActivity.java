...
import com.project.complaint.databinding.ActivityMapsBinding;
...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        LatLng latLngUser = new LatLng(gpsTracker.latitude, gpsTracker.longitude);

        MarkerOptions markerOptionsUser = new MarkerOptions();
        markerOptionsUser.position(latLngUser);
        markerOptionsUser.title("User Location");

        CameraPosition cameraPosition = new CameraPosition.Builder().target(latLngUser).zoom(18).build();
